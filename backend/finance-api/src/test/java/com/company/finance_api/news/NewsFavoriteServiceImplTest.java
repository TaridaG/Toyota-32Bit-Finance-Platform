package com.company.finance_api.news;
...
import com.company.finance_api.service.impl.NewsFavoriteServiceImpl;
...
class NewsFavoriteServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        newsFavoriteService = new NewsFavoriteServiceImpl(
    ...
    void addFavorite_reactivatesInactiveRow() {
        ...
        inactive.deactivate();
        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        when(newsFavoriteRepository.findByUserIdAndNewsId(userId, 7L)).thenReturn(Optional.of(inactive));
