...
public class PortfolioInsightServiceImpl implements PortfolioInsightService {
    ...
    public List<InsightDto> analyze(
        ...
        assets.stream()
                ...
                .max(Comparator.comparing(PortfolioValuationAssetDto::weight)
                        .thenComparing(PortfolioValuationAssetDto::instrumentId))
                .ifPresent(a -> insights.add(new InsightDto(
                        "HIGH_CONCENTRATION",
        ...
        for (Map.Entry<String, BigDecimal> e : segmentBreakdownPercent.entrySet()) {
            if (e.getValue().compareTo(SEVENTY) > 0) {
                insights.add(new InsightDto(
                        ...
                        "Segment " + e.getKey() + " represents " + e.getValue().stripTrailingZeros().toPlainString()
                                + "% of portfolio value.",
                        InsightSeverity.HIGH,
                        Map.of(
                                "segment", e.getKey(),
        ...
        Optional<PortfolioValuationAssetDto> largest = assets.stream()
                .max(Comparator.comparing(PortfolioValuationAssetDto::weight)
                        .thenComparing(PortfolioValuationAssetDto::instrumentId));
        largest.filter(a -> a.weight().compareTo(BigDecimal.ZERO) > 0)
                .ifPresent(a -> insights.add(new InsightDto(
                        "LARGEST_POSITION",
                        "Largest position instrumentId=" + a.instrumentId() + " weight="
        ...
        List<PortfolioValuationAssetDto> priced = assets.stream().filter(PortfolioValuationAssetDto::hasPrice).toList();
        if (!priced.isEmpty()) {
            PortfolioValuationAssetDto gainer = priced.stream()
                    .max(Comparator.comparing(PortfolioValuationAssetDto::pnl)
                            .thenComparing(PortfolioValuationAssetDto::instrumentId))
                    .orElseThrow();
            insights.add(new InsightDto(
                    "TOP_GAINER",
                    "Top PnL instrumentId=" + gainer.instrumentId() + " pnl=" + gainer.pnl().stripTrailingZeros().toPlainString() + ".",
                    InsightSeverity.LOW,
                    Map.of(
            ...
            if (assets.size() > 1) {
                PortfolioValuationAssetDto loser = priced.stream()
                        .min(Comparator.comparing(PortfolioValuationAssetDto::pnl)
                                .thenComparing(PortfolioValuationAssetDto::instrumentId))
                        .orElseThrow();
                insights.add(new InsightDto(
                        "TOP_LOSER",
                        "Weakest PnL instrumentId=" + loser.instrumentId() + " pnl=" + loser.pnl().stripTrailingZeros().toPlainString() + ".",
                        InsightSeverity.LOW,
                        Map.of(
    ...
    private void recordInsightMetrics(List<InsightDto> insights) {
        for (InsightDto insight : insights) {
            meterRegistry.counter(
                    "portfolio_insight_generated_total",
                    Tags.of("type", insight.type())
            ).increment();
        }
    }
}
