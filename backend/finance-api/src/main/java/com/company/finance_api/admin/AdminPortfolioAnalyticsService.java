...
import com.company.finance_api.admin.dto.AdminPortfolioAnalyticsSummaryDto;
import com.company.finance_api.admin.dto.AdminPortfolioDailyCreationDto;
import com.company.finance_api.admin.dto.AdminRecentPortfolioRowDto;
...
public class AdminPortfolioAnalyticsService {
    ...
    @Transactional(readOnly = true)
    public AdminPortfolioAnalyticsDashboardDto dashboardCustom(LocalDate fromInclusive, LocalDate toInclusive) {
        Instant now = Instant.now();
        LocalDate todayUtc = LocalDate.ofInstant(now, UTC);
    ...
    private AdminPortfolioAnalyticsDashboardDto build(
        ...
        double averageOpenLotsPerPortfolio = totalPortfolios > 0 ? (double) openLots / totalPortfolios : 0.0;
        ...
        long prevWeekCreated = externalPortfolioRepository.countCreatedBetween(
        ...
        LocalDateTime prevMonthToExclusive = prevMonth.plusMonths(1).atDay(1).atStartOfDay(UTC).toLocalDateTime();
        long prevMonthCreated = externalPortfolioRepository.countCreatedBetween(
        ...
        long createdYesterday = externalPortfolioRepository.countCreatedBetween(
                Timestamp.valueOf(yStart), Timestamp.valueOf(yEnd));
        long createdDayBefore = externalPortfolioRepository.countCreatedBetween(
                Timestamp.valueOf(y2Start), Timestamp.valueOf(y2End));

        long impliedPrior = Math.max(0L, totalPortfolios - createdYesterday);
        double totalVsPrior = AdminPortalUserMetricsService.weekOverWeekPercent(totalPortfolios, impliedPrior);

        AdminPortfolioAnalyticsSummaryDto summary = new AdminPortfolioAnalyticsSummaryDto(
                totalPortfolios,
                prevWeekCreated,
                prevMonthCreated,
        ...
        List<ExternalPortfolio> recentRows =
                externalPortfolioRepository.findAllWithUserOrderByCreatedAtDesc(PageRequest.of(0, RECENT_LIMIT));
        ...
        for (ExternalPortfolio p : recentRows) {
            String owner = p.getUser() != null && p.getUser().getUsername() != null
                    ...
                    : "—";
            recent.add(new AdminRecentPortfolioRowDto(
                    p.getId(),
                    p.getName(),
                    p.getBaseCurrency(),
                    owner,
