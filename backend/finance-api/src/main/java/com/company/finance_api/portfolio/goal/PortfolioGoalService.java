...
public class PortfolioGoalService {
    ...
    @Transactional
    public PortfolioGoalsViewResponse upsertPortfolioValueGoal(UpsertPortfolioValueGoalRequest request, String targetCurrency) {
        UUID userId = currentUserResolver.getCurrentUserId();
        ...
        PortfolioGoal goal = portfolioGoalRepository
    ...
    @Transactional
    public PortfolioGoalsViewResponse upsertProfitGoal(UpsertProfitGoalRequest request, String targetCurrency) {
        UUID userId = currentUserResolver.getCurrentUserId();
        Long portfolioId = request.getPortfolioId();
        ExternalPortfolio portfolio = resolvePortfolioForUser(userId, portfolioId);
        User user = userRepository.findById(userId)
        ...
        if (mode == ProfitTargetMode.PERCENT) {
            if (request.getTargetPercent() == null || request.getTargetPercent().compareTo(BigDecimal.ZERO) <= 0) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "targetPercent is required for percent profit goals");
            }
        } else if (request.getTargetAmount() == null || request.getTargetAmount().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "targetAmount is required for absolute profit goals");
        }
        ...
        goal.setTargetAmount(mode == ProfitTargetMode.ABSOLUTE
                ? request.getTargetAmount().setScale(4, RoundingMode.HALF_UP)
                : null);
        goal.setTargetPercent(mode == ProfitTargetMode.PERCENT
    ...
    private PortfolioGoalsViewResponse buildView(UUID userId, Long portfolioId, String targetCurrency) {
        PortfolioOverviewResponse overview = portfolioOverviewService.getMyOverview(targetCurrency, portfolioId);
        String currency = overview.currency() != null ? overview.currency() : "USD";

        PortfolioGoal portfolioGoal = portfolioGoalRepository
                .findForScopeAndType(userId, portfolioId, GoalType.PORTFOLIO_VALUE)
                .orElse(null);
        PortfolioGoal profitGoal = portfolioGoalRepository
                .findForScopeAndType(userId, portfolioId, GoalType.PROFIT)
                .orElse(null);
    ...
    private PortfolioGoalCardDto toPortfolioValueCard(PortfolioGoal goal, PortfolioOverviewResponse overview) {
        ...
        BigDecimal ratio = target.compareTo(BigDecimal.ZERO) > 0
    ...
    private PortfolioGoalCardDto toProfitCard(PortfolioGoal goal, PortfolioOverviewResponse overview) {
        BigDecimal pnl = safe(overview.totalPnl());
        BigDecimal pnlPct = overview.totalPnlPercent() != null ? overview.totalPnlPercent() : BigDecimal.ZERO;
        if (goal == null) {
        ...
        if (goal.getProfitTargetMode() == ProfitTargetMode.PERCENT) {
            ...
            if (pnl.compareTo(BigDecimal.ZERO) < 0) {
                variant = "PROFIT_LOSS";
                ratio = targetPct.compareTo(BigDecimal.ZERO) > 0
        ...
        if (pnl.compareTo(BigDecimal.ZERO) < 0) {
            variant = "PROFIT_LOSS";
            ratio = targetAmt.compareTo(BigDecimal.ZERO) > 0
    ...
    private ExternalPortfolio resolvePortfolioForUser(UUID userId, Long portfolioId) {
        ...
        return externalPortfolioRepository.findByIdAndUserId(portfolioId, userId)
    ...
    private static ProfitTargetMode parseProfitMode(String raw) {
        ...
        return switch (normalized) {
            ...
            default -> throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Unsupported profitTargetMode");
        };
    }
