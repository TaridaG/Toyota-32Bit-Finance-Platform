...
public class AdminPortalPortfolioMetricsService {
    ...
    private static final Logger log = LoggerFactory.getLogger(AdminPortalPortfolioMetricsService.class);

    private final ExternalPortfolioRepository externalPortfolioRepository;

    public AdminPortalPortfolioMetricsService(ExternalPortfolioRepository externalPortfolioRepository) {
    ...
    public AdminPortalPortfolioMetricsDto snapshot() {
        ...
        try {
            ...
            log.warn("Portfolio velocity/daily buckets skipped; totalPortfolios={} is still valid", total, ex);
        }

        double wow = AdminPortalUserMetricsService.weekOverWeekPercent(last7, prev7);
        return new AdminPortalPortfolioMetricsDto(total, last7, prev7, wow, daily, updatesDaily, now, true);
    }
    ...
    private long countCreatedBetweenInstants(Instant from, Instant to) {
        ...
        return externalPortfolioRepository.countCreatedBetween(Timestamp.valueOf(start), Timestamp.valueOf(end));
    }
    ...
    private List<Integer> dailyNewPortfoliosUtcLast7Days(Instant now) {
        ...
        for (int i = 6; i >= 0; i--) {
            ...
            out.add((int) Math.min(
                    Integer.MAX_VALUE,
                    externalPortfolioRepository.countCreatedBetween(Timestamp.valueOf(start), Timestamp.valueOf(end))));
        }
        return out;
    }
