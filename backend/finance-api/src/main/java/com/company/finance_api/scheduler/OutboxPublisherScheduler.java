        ...
        for (OutboxEvent e : batch) {
            try {
                ...
                if (e.getMessageKey() == null || e.getMessageKey().isBlank()) {
                    kafkaTemplate.send(e.getTopic(), payload)
                            .get(10, TimeUnit.SECONDS);
                } else {
                    kafkaTemplate.send(e.getTopic(), e.getMessageKey(), payload)
                            .get(10, TimeUnit.SECONDS);
                }
                ...
                if (e.getAttempts() + 1 >= maxAttempts) {
                    e.markDead(err);
                    log.error("Outbox DEAD id={}, topic={}, type={}, error={}", e.getId(), e.getTopic(), e.getEventType(), err);
                } else {
                    Instant next = Instant.now().plus(backoffSeconds, ChronoUnit.SECONDS);
                    e.markRetry(err, next);
                    log.warn("Outbox RETRY id={}, attempt={}, nextAt={}, error={}",
                            e.getId(), e.getAttempts(), next, err);
                }
            }
        }
