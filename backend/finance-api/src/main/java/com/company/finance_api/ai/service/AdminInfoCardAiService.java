...
public class AdminInfoCardAiService {
    ...
    public InfoCardAiContentResponse complete(CompleteInfoCardAiRequest request) {
        ...
        String fingerprint = request.language() + "|" + request.title() + "|" + request.shortDescription();

        try {
            String json = openAiClient.requestStructuredJson(
    ...
    public InfoCardAiContentResponse translate(TranslateInfoCardAiRequest request) {
        ...
        if (request.sourceLanguage().equals(request.targetLanguage())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "sourceLanguage and targetLanguage must differ");
        }

        String model = aiProperties.getOpenai().getModels().getTranslation();
        String createdBy = currentPrincipalName();
        String fingerprint = request.sourceLanguage() + "->" + request.targetLanguage() + "|" + request.sourceContent().title();
    ...
    private InfoCardAiContentResponse filterCompleteResponse(InfoCardAiContentResponse parsed, List<String> fields) {
