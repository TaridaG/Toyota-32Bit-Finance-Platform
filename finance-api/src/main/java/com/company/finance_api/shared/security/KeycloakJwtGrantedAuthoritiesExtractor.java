...
/**
 * Keycloak realm / client role'lerini Spring {@code GrantedAuthority} ({@code ROLE_*} prefix)
 * olarak map eder; API gateway extractor ile hizalıdır.
 */
public final class KeycloakJwtGrantedAuthoritiesExtractor {

  private KeycloakJwtGrantedAuthoritiesExtractor() {}

  /** JWT claim'lerinden birleşik Spring Security authority koleksiyonu üretir. */
  ...
  private static void addRolesFromScope(Jwt jwt, Set<String> roles) {
    ...
    Set<String> skip = Set.of("openid", "profile", "email", "address", "phone", "offline_access");
