...
import com.company.finance_api.admin.infrastructure.http.dto.AdminRecentRegistrationRowDto;
import com.company.finance_api.admin.infrastructure.http.dto.AdminUserAnalyticsDashboardDto;
import com.company.finance_api.admin.infrastructure.http.dto.AdminUserAnalyticsSummaryDto;
import com.company.finance_api.admin.infrastructure.http.dto.AdminUserDailyRegistrationDto;
...
/** Enterprise 'Total users' analytics dashboard verisini users tablosundan UTC hizalı üretir. */
@Service
public class AdminUserAnalyticsService {
  ...
  /** Özel aralıklar için azami dahil gün sayısı (kötüye kullanım koruması). */
  ...
  /** Preset pencere için kullanıcı analytics dashboard DTO'sunu üretir. */
  ...
  /**
   * Dahil UTC takvim tarihleri {@code [fromInclusive, toInclusive]}; bugün UTC ile sınırlı.
   *
   * @throws IllegalArgumentException aralık boş, ters, çok uzun veya geçmişte çok eskiyse
   */
  @Transactional(readOnly = true)
  public AdminUserAnalyticsDashboardDto dashboardCustom(
  ...
  private AdminUserAnalyticsDashboardDto build(
    ...
    long currentPeriod =
        userRepository.countCreatedInRangeExcludingPendingDeletion(chartFrom, chartToExclusive);
    long previousPeriod =
        userRepository.countCreatedInRangeExcludingPendingDeletion(prevFrom, prevToExclusive);
    double growthPeriod =
        AdminPortalUserMetricsService.weekOverWeekPercent(currentPeriod, previousPeriod);
    ...
    long newUsersYesterday =
        userRepository.countCreatedInRangeExcludingPendingDeletion(yStart, yEnd);
    long newUsersDayBefore =
        userRepository.countCreatedInRangeExcludingPendingDeletion(y2Start, y2End);
    ...
    long newUsersPreviousIsoWeekUtc =
        userRepository.countCreatedInRangeExcludingPendingDeletion(
            prevIsoWeekFrom, prevIsoWeekToExclusive);
    ...
    long newUsersPreviousCalendarMonthUtc =
        userRepository.countCreatedInRangeExcludingPendingDeletion(
            prevMonthFrom, prevMonthToExclusive);

    long impliedRosterPriorUtcDay = Math.max(0L, totalUsers - newUsersYesterday);
    double totalUsersVsPriorDayPercentApprox =
        AdminPortalUserMetricsService.weekOverWeekPercent(totalUsers, impliedRosterPriorUtcDay);
    ...
    for (int i = 0; i < span; i++) {
      ...
      newCounts.add(
          (int)
              Math.min(
                  Integer.MAX_VALUE,
                  userRepository.countCreatedInRangeExcludingPendingDeletion(start, end)));
      delCounts.add(
          (int)
              Math.min(
                  Integer.MAX_VALUE, userRepository.countDeletionRequestedInRange(start, end)));
    }
    ...
    for (int i = 0; i < span; i++) {
      ...
      daily.add(
          new AdminUserDailyRegistrationDto(
              d.toString(), nu, delCounts.get(i), rolling.get(i), delta));
      if (d.equals(todayUtc)) {
        todayNew = nu;
      }
    }

    long deletedAccountsTotal = adminPortalRosterCounterService.deletedAccountsTotal();
    long frozenAccountsNow = userRepository.countFrozenRoster();

    AdminUserAnalyticsSummaryDto summary =
        new AdminUserAnalyticsSummaryDto(
            totalUsers,
            last7,
            prev7,
            wow7,
  ...
  private List<AdminRecentRegistrationRowDto> mapRecent() {
    ...
    for (User u : rows) {
      ...
      out.add(
          new AdminRecentRegistrationRowDto(
              u.getId(),
              u.getUsername(),
