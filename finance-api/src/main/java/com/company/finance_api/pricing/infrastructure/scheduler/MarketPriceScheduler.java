...
/** Demo profile'da aktif enstrümanlar için market fiyatlarını periyodik günceller. */
@Slf4j
@Component
@Profile("demo-prices")
...
public class MarketPriceScheduler {
  ...
  public void refreshMarketPrices() {
    ...
    PriceProvider provider = providerResolver.resolve(PriceType.MARKET);

    for (Instrument instrument : instruments) {

      InstrumentPrice price = provider.fetchLatestPrice(instrument);

      priceService.savePrice(price);

      log.debug("Market price updated: {} = {}", instrument.getSymbol(), price.getPrice());
    }
  }
}
