...
/** Yarı yıllık kuponlu eurobond için yield-to-maturity hesaplar. */
public final class SemiAnnualBondYieldSolver {
  ...
  public static BigDecimal annualPercentFromCleanPrice(
      BigDecimal cleanPricePct,
      BigDecimal annualCouponPct,
      LocalDate asOf,
  ...
  /**
   * Inverse of {@link #annualPercentFromCleanPrice}: finds clean price (100 par) matching a target
   * annual yield %.
   */
  public static BigDecimal cleanPriceFromAnnualYieldPercent(
    ...
    for (int i = 0; i < 55; i++) {
      ...
      BigDecimal yMid =
          annualPercentFromCleanPrice(BigDecimal.valueOf(mid), annualCouponPct, asOf, maturityDate);
      if (yMid == null) {
        return null;
      }
