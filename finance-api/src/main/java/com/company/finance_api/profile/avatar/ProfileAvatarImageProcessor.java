...
/** Yüklenen görseli kare JPEG'e ölçekler ve sıkıştırır. */
@Component
public class ProfileAvatarImageProcessor {
  ...
  /** Ham görseli kare sınırlı JPEG'e dönüştürür. */
  public byte[] toOptimizedJpeg(byte[] raw) {
    ...
    if (source == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Unsupported or corrupt image format");
    }
  ...
  private byte[] writeJpeg(BufferedImage image, float quality) {
    ...
    try (ImageOutputStream ios = ImageIO.createImageOutputStream(bos)) {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Could not encode avatar image", e);
    } finally {
      writer.dispose();
    }
