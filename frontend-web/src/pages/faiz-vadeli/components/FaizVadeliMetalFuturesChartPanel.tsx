...
import { fetchMetalFuturesHistory } from '../api/metalFuturesMarketApi'
import type { MarketHistoryPoint } from '../api/tahvilMarketApi'
import type { MetalFuturesSymbol } from './FaizVadeliMetalFuturesStatCard'
...
export function FaizVadeliMetalFuturesChartPanel({
  symbol,
  onBack,
}: {
  ...
  useEffect(() => {
    ...
    setPoints([])
    fetchMetalFuturesHistory(symbol)
      .then((r) => {
        if (cancelled) return
        setPoints(r ?? [])
        setLoadState('ready')
    ...
    <div className="fi-faiz-panel fi-faiz-panel--chart fi-faiz-panel--policy-chart">
      <div className="fi-faiz-panel-head-row">
        <h3 className="fi-faiz-panel-title">
          {t('faizVadeliPage.metalFutures.chartTitle', { symbol })}
        </h3>
        <button type="button" className="fi-faiz-panel-back" onClick={onBack}>
          {t('faizVadeliPage.metalFutures.chartBackToYield')}
        </button>
      </div>

      <div className="fi-faiz-policy-chart-y-wrap">
        <span className="fi-faiz-policy-chart-y-label">{t('faizVadeliPage.metalFutures.chartAxisY')}</span>
        <div className="fi-faiz-chart-wrap fi-faiz-policy-panel-chart-wrap">
          {loadState === 'loading' ? (
            <div
              ...
              aria-busy="true"
              aria-label={t('faizVadeliPage.metalFutures.chartLoading')}
            />
          ) : null}

          {loadState === 'error' ? (
            <div className="fi-faiz-policy-panel-state">
              <p>{t('faizVadeliPage.metalFutures.chartError')}</p>
              <button type="button" className="profile-settings-btn-secondary" onClick={() => setRetryNonce((n) => n + 1)}>
                {t('faizVadeliPage.metalFutures.chartRetry')}
              </button>
            </div>
          ) : null}

          {loadState === 'ready' && lineData.length === 0 ? (
            <p className="fi-faiz-policy-panel-state">{t('faizVadeliPage.metalFutures.chartEmpty')}</p>
          ) : null}
      ...
      <p className="fi-faiz-chart-foot">{t('faizVadeliPage.metalFutures.chartFoot')}</p>
    </div>
  )
}
