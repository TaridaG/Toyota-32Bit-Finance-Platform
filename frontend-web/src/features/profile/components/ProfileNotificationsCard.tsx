...
export function ProfileNotificationsCard({ profile, onUpdated }: Props) {
  ...
  const [product, setProduct] = useState(profile.notifyProductUpdates)
  ...
  useEffect(() => {
    ...
    setProduct(profile.notifyProductUpdates)
  }, [profile.notifySecurityAlerts, profile.notifyProductUpdates])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setBusy(true)
    try {
      const next = await updatePortalNotifications(security, product)
      onUpdated(next)
      setSuccess(true)
    ...
    <ProfileSettingsPanel kicker={t('profileSettings.kickerNotifications')} summary={null} expanded collapsible={false}>
      ...
      <form className="profile-settings-form" onSubmit={(ev) => void handleSubmit(ev)}>
        ...
        <label className="profile-settings-toggle">
          ...
          <span>
            <span className="profile-settings-toggle-title">{t('profileSettings.notifyProduct')}</span>
            <span className="profile-settings-toggle-desc">{t('profileSettings.notifyProductDesc')}</span>
          </span>
        </label>
